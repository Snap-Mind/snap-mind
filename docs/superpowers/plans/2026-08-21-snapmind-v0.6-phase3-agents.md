...
- `electron/db/schema.ts` — *modified.* Adds the `agents` and `hotkeys` tables plus their row types.
- `electron/db/migrations/0001_*.sql` — *new.* Creates both tables and seeds the Default agent and four hotkey rows.
- `electron/AgentsService.ts` — *new.* Agent CRUD and `config_json` packing. Mirrors `ProvidersService`.
- `electron/HotkeysService.ts` — *new.* Hotkey read and patch. No create or delete.
- `electron/db/importAgents.ts` — *new.* One-shot legacy import. Mirrors `electron/db/import.ts`.
- `electron/SettingsService.ts` — *modified.* Loses every hotkey member and `resolveAndPersistDefaults`.
- `electron/IpcHandlers.ts` — *modified.* Swaps the three hotkey channels for `hotkeys:list` / `hotkeys:update` and adds four `agents:*` channels.
- `main.ts` — *modified.* Constructs both services, reads hotkeys from the DB, and dispatches on `mode`.

**Renderer**

- `src/types/agent-dto.ts`, `src/types/hotkey-dto.ts` — *new.* Shared DTOs, mirroring `src/types/provider-dto.ts`.
- `src/stores/useAgentsStore.ts`, `src/stores/useHotkeysStore.ts` — *new.*
- `src/services/agentResolver.ts` — *new.* The single resolution helper used by the chat store and the badge.
- `src/services/AIService.ts` — *modified.* Takes a resolved provider, model, and params instead of `settings.chat`.
- `src/stores/useChatStore.ts` — *modified.* `activeAgentId` replaces the provider/model pair.
- `src/pages/ChatPopup/ChatPopup.tsx` — *modified.* Picker out, agent badge in.
- `src/pages/Settings/Agents/SettingsAgents.tsx`, `AgentEditor.tsx` — *new.*
- `src/pages/Settings/SettingsHotkeys.tsx` — *modified.* Agent dropdown replaces the prompt textarea.
- `src/pages/Settings/SettingsChat.tsx` — *deleted.*
...
function resolveActiveAgent(agentId: number | null): AgentResolution {
  return resolveAgent(agentId, useAgentsStore.getState().agents, useProvidersStore.getState().providers);
}
...
- In "What each phase deliberately *doesn't* do", extend the Phase 3 bullet: it still does not touch the provider layer, and it adds no tool support.
